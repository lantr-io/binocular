...
/** Configuration for Cardano wallet
  *
  * Supports mnemonic phrase-based wallet configuration.
  * Can be configured via:
  * 1. application.conf file
  * 2. Environment variables
  * 3. Direct construction
  *
  * SECURITY WARNING: Storing mnemonic phrases in config files or environment
  * variables is convenient for development but NOT recommended for production.
  * Consider using hardware wallets or secure key management systems for mainnet.
  *
  * Examples:
  * {{{
...
case class WalletConfig(
  ...
  def createAccount(): Either[String, Account] = {
    Try {
      ...
      if (!List(12, 15, 18, 21, 24).contains(mnemonicWords.length)) {
  ...
  def validate(): Either[String, Unit] = {
    if (mnemonic.isEmpty || mnemonic == "changeme") {
      Left("Wallet mnemonic must be configured. Set WALLET_MNEMONIC or configure binocular.wallet.mnemonic")
    } else {
      val words = mnemonic.trim.split("\\s+")
      if (!List(12, 15, 18, 21, 24).contains(words.length)) {
...
object WalletConfig {
  ...
  def fromConfig(config: Config = ConfigFactory.load()): Either[String, WalletConfig] = {
    Try {
      ...
      val networkStr = if (walletConfig.hasPath("network")) {
        walletConfig.getString("network")
      } else if (config.hasPath("binocular.cardano.network")) {
      ...
      case Failure(ex) => Left(s"Failed to load wallet config from application.conf: ${ex.getMessage}")
    }
  }
  ...
  /** Create wallet config from mnemonic phrase
    *
    * @param mnemonic 12/15/18/21/24-word mnemonic phrase
    * @param network Cardano network (defaults to preprod for safety)
    */
  def fromMnemonic(
      mnemonic: String,
