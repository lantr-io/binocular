...
case class CardanoConfig(
  ...
  def createBackendService(): Either[String, BackendService] = {
    backend match {
      case CardanoBackend.Blockfrost =>
        if (blockfrost.projectId.isEmpty || blockfrost.projectId == "changeme") {
          Left("Blockfrost backend requires valid project ID. Set BLOCKFROST_PROJECT_ID or configure binocular.cardano.blockfrost.project-id")
        } else {
          Try {
            new BFBackendService(blockfrost.apiUrl, blockfrost.projectId)
          } match {
            case Success(service) => Right(service)
            case Failure(ex) => Left(s"Failed to create Blockfrost backend: ${ex.getMessage}")
          }
        }
  ...
  def validate(): Either[String, Unit] = {
    backend match {
      case CardanoBackend.Blockfrost =>
        if (blockfrost.apiUrl.isEmpty) {
          Left("Blockfrost API URL cannot be empty")
        } else if (blockfrost.projectId.isEmpty || blockfrost.projectId == "changeme") {
  ...
  override def toString: String = {
    val maskedBlockfrostId = if (blockfrost.projectId.length > 8)
      blockfrost.projectId.take(8) + "***"
    else
      "***"
...
object CardanoNetwork {
  def fromString(s: String): Either[String, CardanoNetwork] = s.toLowerCase match {
    ...
    case _ => Left(s"Unknown Cardano network: $s. Valid options: mainnet, preprod, preview, testnet")
  }
}
...
object CardanoConfig {
  ...
  def fromConfig(config: Config = ConfigFactory.load()): Either[String, CardanoConfig] = {
    Try {
      ...
      case Failure(ex) => Left(s"Failed to load Cardano config from application.conf: ${ex.getMessage}")
    }
  }
  ...
  def load(): Either[String, CardanoConfig] = {
    ...
    envConfig match {
      case Right(cfg) if cfg.backend == CardanoBackend.Blockfrost &&
  ...
  /** Create Blockfrost configuration for a specific network
    *
    * @param network Cardano network (mainnet, preprod, preview, testnet)
  ...
  /** Create Ogmios configuration for local node
    ...
    * @param url Ogmios WebSocket URL (default: http://localhost:1337)
    */
  def ogmios(
      network: CardanoNetwork,
