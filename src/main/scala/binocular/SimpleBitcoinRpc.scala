...
class SimpleBitcoinRpc(config: BitcoinNodeConfig)(using ec: ExecutionContext) {
  ...
  private def call(method: String, params: ujson.Value = ujson.Arr()): Future[ujson.Value] = Future {
    requestId += 1
    ...
    try {
      ...
      case e: java.net.http.HttpTimeoutException =>
        val elapsed = System.currentTimeMillis() - startTime
        throw new RuntimeException(s"RPC timeout after ${elapsed}ms calling $method with params $params", e)
      case e: java.net.ConnectException =>
        throw new RuntimeException(s"Connection failed to ${config.url}: ${e.getMessage}", e)
      case e: Exception =>
        val elapsed = System.currentTimeMillis() - startTime
        throw new RuntimeException(s"RPC call failed after ${elapsed}ms: ${e.getMessage}", e)
    }
  }
  ...
  def getBlockHeader(hash: String): Future[BlockHeaderInfo] = {
    call("getblockheader", ujson.Arr(hash, true)).map { result =>
      ...
      val requiredFields = Seq("hash", "height", "version", "merkleroot", "time", "nonce", "bits", "difficulty")
      requiredFields.foreach { field =>
        if (!result.obj.contains(field)) {
          throw new RuntimeException(s"Missing required field '$field' in getblockheader response for hash $hash")
        }
      }
  ...
  def getBlock(hash: String): Future[BlockInfo] = {
    call("getblock", ujson.Arr(hash, 2)).map { result =>
      // Validate required fields are present
      val requiredFields = Seq("hash", "height", "version", "merkleroot", "time", "nonce", "bits", "difficulty", "tx")
      requiredFields.foreach { field =>
        if (!result.obj.contains(field)) {
          throw new RuntimeException(s"Missing required field '$field' in getblock response for hash $hash")
        }
      }
