...
object BitcoinHeaderFixtures {
    ...
    /** Generate fixture by fetching headers from Bitcoin RPC
      ...
      *
      * @param name
      *   fixture name (used for filename)
      * @param startHeight
      *   starting block height
      * @param count
      *   number of blocks to fetch
      * @param outputDir
      *   directory to save fixture (default: src/test/resources/fixtures)
      * @return
      *   Future that completes when fixture is saved
      */
    def generateFixture(
        name: String,
        startHeight: Int,
        ...
        def fetchHeaders(
            height: Int,
            remaining: Int,
            acc: List[HeaderFixture]
        ): Future[List[HeaderFixture]] = {
            if remaining <= 0 then Future.successful(acc.reverse)
            else
                for
                    ...
                    _ = print(
                      s"\rFetching header at height $height... (${count - remaining + 1}/$count)"
                    )
    ...
    /** Load fixture from JSON file
      *
      * @param name
      *   fixture name (without .json extension)
      * @param fixturesDir
      *   directory containing fixtures
      * @return
      *   HeaderSequenceFixture loaded from file
      */
    def loadFixture(
        name: String,
    ...
    /** Create initial ChainState from first header in sequence
      ...
      *
      * @param fixture
      *   the header sequence fixture
      * @return
      *   ChainState initialized from first header
      */
    ...
    def main(args: Array[String]): Unit = {
        ...
        val bitcoindUser =
            sys.env.getOrElse("bitcoind_rpc_user", sys.error("bitcoind_rpc_user not set"))
        val bitcoindPassword =
            sys.env.getOrElse("bitcoind_rpc_password", sys.error("bitcoind_rpc_password not set"))
        ...
        given BitcoindRpcAppConfig = BitcoindRpcAppConfig(
          ...
          authCredentinalsOpt =
              Some(BitcoindAuthCredentials.PasswordBased(bitcoindUser, bitcoindPassword))
        )
