...
class ChainworkSelectionTest extends AnyFunSuite {
    ...
    test("best tip follows the higher-chainwork honest branch, not the longer rogue branch") {
        ...
        val eve =
            Blocks(PList.from((1 to 5).map(i => block(s"e$i")).toList), chainwork = 5, next = End)
        ...
        assert(
          tip.contains(block("a2").hash),
          "Alice's heavier branch must win despite Eve having more blocks"
        )
