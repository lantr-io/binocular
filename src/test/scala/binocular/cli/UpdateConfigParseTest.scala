...
class UpdateConfigParseTest extends AnyFunSuite {
    ...
    test("every parameter flag reaches ParamEdits") {
        ...
        assert(cmd.bridgeStatePolicy.isEmpty && cmd.tmScriptHash.isEmpty)
        assert(cmd.pegInWithdrawHash.isEmpty && cmd.pegOutWithdrawHash.isEmpty)
    }

    test("a params-only update needs no hash swap") {
        val cmd = updateConfig("update-config", "--fee-rate", "5")
        assert(cmd.bridgeStatePolicy.isEmpty && cmd.tmScriptHash.isEmpty)
        assert(cmd.params.feeRateSatPerVb.contains(BigInt(5)))
    }

    test("the rev-5.4 hash swap flags all parse") {
        val cmd = updateConfig(
          "update-config",
          "--bridge-state-policy",
          "ab" * 28,
        ...
        assert(cmd.bridgeStatePolicy.contains("ab" * 28))
        assert(cmd.tmScriptHash.contains("bc" * 28))
        assert(cmd.pegInWithdrawHash.contains("cd" * 28))
        assert(cmd.pegOutWithdrawHash.contains("de" * 28))
    ...
    test("update-config with no edits at all still parses (the command reports it)") {
        ...
        assert(cmd.params.isEmpty && cmd.bridgeStatePolicy.isEmpty && cmd.tmScriptHash.isEmpty)
    }
