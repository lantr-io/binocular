...
/** Utility to fetch real Bitcoin block data from RPC for integration testing.
  *
  * Uses the existing HeaderSyncWithRpc to connect to Bitcoin RPC. Can work with QuickNode,
  * GetBlock, or local Bitcoin Core node.
  *
  * Usage:
  * {{{
...
object BitcoinDataFetcher {
    ...
    def main(args: Array[String]): Unit = {
        ...
        // Parse arguments
        val (blockHeights, outputDir) = if args(0) == "--range" then {
            ...
            if start >= end then {
                System.err.println(
                  s"Error: start height ($start) must be less than end height ($end)"
                )
                sys.exit(1)
            }
        ...
        try {
            val timeout = if blockHeights.length > 50 then 15.minutes else 10.minutes
    ...
    def fetchBlockData(
        ...
        for {
            ...
            _ <- Future.successful(
              println(s"  ✓ Generated ${merkleTestCase.testCases.length} merkle proof test cases")
            )
            ...
            case (futureResults, (height, idx)) =>
                for {
                    ...
                    _ = if idx > 0 && idx % 10 == 0 then {
                        ...
                        println(
                          s"  Time elapsed: ${elapsed.round}s, avg: ${avgTime.round}s/block, est. remaining: ${remaining.round}s"
                        )
                    }
